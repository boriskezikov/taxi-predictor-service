...
def init_preprocessing(recalculate=True):
    ...
    # unique_binswithPickup_Jan_2016: list = fun.getUniqueBinsWithPickups(jan_2016_data, n_clusters)
    ...
    if recalculate:
        ...
        jan_2016_fillZero = spark.read.csv(PROCESSED_DATA_DRIVE + "jan_2016_fillZero_{}.csv".format(datetime.now().date()))

    regionWisePickup_Jan_2016 = []
    for i in range(n_clusters):
        regionWisePickup_Jan_2016.append(jan_2016_fillZero[4464 * i:((4464 * i) + 4464)])

    TruePickups, lat, lon, day_of_week, feat = fun.compute_pickups(regionWisePickup_Jan_2016, n_clusters)
