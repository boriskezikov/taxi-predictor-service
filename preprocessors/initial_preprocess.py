...
def init_preprocessing(recalculate=False):
    ...
    #
    # # temp for testing
    # new_frame_cleaned.write.csv(HDFS_HOST + "preprocessed/new_frame_cleaned.csv", header=True, mode="overwrite")
    # new_frame_cleaned2.write.csv(HDFS_HOST + "preprocessed/new_frame_cleaned2.csv", header=True, mode="overwrite")
    # new_frame_cleaned = spark.read.csv(HDFS_HOST + "preprocessed/new_frame_cleaned.csv")
    # new_frame_cleaned2 = spark.read.csv(HDFS_HOST + "preprocessed/new_frame_cleaned2.csv")
    ...
    # k_means = KMeansModelCustom(HDFS_HOST, use_pretrained=False)
    ...
    # jan_2016_timeBin_groupBy.write.csv(HDFS_HOST + "preprocessed/jan_2016_timeBin_groupBy.csv", header=True,
    #                                    mode="overwrite")
    jan_2016_timeBin_groupBy = spark.read.csv(HDFS_HOST + "preprocessed/jan_2016_timeBin_groupBy.csv", header=True,
                                              inferSchema=True)

    if recalculate:
        jan_2016_fillZero = fun.fill_missing_tbins_with_zero(jan_2016_timeBin_groupBy, n_clusters)
        jan_2016_fillZero.write.csv(PROCESSED_DATA_DRIVE + "jan_2016_fillZero_{}.csv".format(datetime.now().date()))
    else:
    ...
    predicted_pickup_values_list = fun.compute_predicted_pickup_values(jan_2016_fillZero, n_clusters)
